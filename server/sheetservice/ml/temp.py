...
def law_predict(input_dict):
    with open('./pickles/law.pkl', 'rb') as f:
...
def media_predict(input_dict):
    with open('./pickles/media_communication_pipeline.pkl', 'rb') as f:
        pipeline = pickle.load(f)
    try:
...
def sports_predict(input_dict):
    with open("./pickles/sports.pkl", "rb") as file:
...
def tech_predict(input_dict):
    with open("./pickles/tech.pkl", "rb") as file:
        pipeline = pickle.load(file)
    relevant_features = [
        'Mathematics',
        'Coding',
...
def predict(input_dict = dict):
    with open("./ml/pickles/creativity.pkl", "rb") as file:
    ...
    with open("./pickles/business.pkl", "rb") as file:
    ...
    healthcare_pipeline = joblib.load('./pickles/healthcare_knn_model.pkl')
    healthcare_features = [
        'Biology',
        'Chemistry',
        'Mathematics',
